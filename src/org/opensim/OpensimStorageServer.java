...
public class OpensimStorageServer implements Storage{
	...
	/** tells if the storage server is heavily loaded  */
	 ...
	 */
	public OpensimStorageServer(String name, double capacity) throws ParameterException {
		if (name == null || name.length() == 0) {
			throw new ParameterException("OpensimStorageServer(): Error - invalid storage name.");
		}

		if (capacity <= 0) {
			throw new ParameterException("OpensimStorageServer(): Error - capacity <= 0.");
		}
	 ...
	 */
	public OpensimStorageServer(double capacity) throws ParameterException {
		if (capacity <= 0) {
			throw new ParameterException("OpensimStorageServer(): Error - capacity <= 0.");
		}
		name = "OpensimStorageServer";
		this.capacity = capacity;
		init();
	}
	...
	/**
	 * OpenSim getFile method
	 * @param fileName the name of the needed file
	 * @return the file with the specified filename
	 */
	public File OpengetFile(String fileName) {
		// check first whether file name is valid or not
		File obj = null;
		if (fileName == null || fileName.length() == 0) {
			Log.printLine(name + ".getFile(): Warning - invalid " + "file name.");
