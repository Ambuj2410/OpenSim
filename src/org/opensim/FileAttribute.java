...
public class FileAttribute {
	...
	public FileAttribute(String fileName, int fileSize) throws ParameterException {
		...
		size = fileSize;
		name = fileName;
		//Log.printLine("Hey3 " + size);
		// set the file creation time. This is absolute time
		/*Date date = new Date();
		Calendar newCal = new Calendar.getInstance();
			newCal=CloudSim.getSimulationCalendar();
		Log.printLine(newCal);
		date=CloudSim.getSimulationCalendar().getTime();
		if (date == null) {
			creationTime = 0;
		} else {
